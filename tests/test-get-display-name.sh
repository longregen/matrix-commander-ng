...
function test1() {
    echo "=== Test 1: get display-name in text format ==="
    matrix-commander-ng --get-display-name $MCRS_OPTIONS
    res=$?
    if [ "$res" == "0" ]; then
        echo "SUCCESS"
    else
...
function test2() {
    ...
    matrix-commander-ng --get-display-name --output json $MCRS_OPTIONS >$fofile
    res=$?
    if [ "$res" == "0" ]; then
